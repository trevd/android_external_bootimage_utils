...
//~ 
    ...
    //~ 
    //~ image->data = (unsigned char *) image->header;
    //~ 
...
//~ unsigned print_boot_image_additional_info(boot_image* image){
    //~ 
    //~ 
    //~ fprintf(stdout,"  data_size        :%08u\n",image->data_size);
    //~ fprintf(stdout,"  header_size      :%08u\n\n",image->header_size);
    //~ 
    //~ fprintf(stdout,"  header_offset    :%08u\n",image->header_offset);
    //~ fprintf(stdout,"  header_padding   :%08u\n\n",image->header_padding);
//~ 
    //~ fprintf(stdout,"  kernel_offset    :%08u\n",image->kernel_offset);
    //~ fprintf(stdout,"  kernel_padding   :%08u\n\n",image->kernel_padding);
    //~ 
    //~ fprintf(stdout,"  ramdisk_offset   :%08u\n",image->ramdisk_offset);
    //~ fprintf(stdout,"  ramdisk_padding  :%08u\n",image->ramdisk_padding);
    //~ 
    //~ if(image->header->second_size > 0){
        //~ fprintf(stdout,"  second_offset    :%08d\n",image->second_offset);
        //~ fprintf(stdout,"  second_padding   :%08d\n",image->second_padding);
    //~ }
    //~ return 0;
//~ }
//~ 
...
//~ 
    //~ fprintf(stdout,"  kernel_size      :%u\n",      image->header->kernel_size);
    //~ fprintf(stdout,"  kernel_addr      :0x%08x\n",  image->header->kernel_addr);
    //~ fprintf(stdout,"  ramdisk_size     :%u\n",      image->header->ramdisk_size);
    //~ fprintf(stdout,"  ramdisk_addr     :0x%08x\n",  image->header->ramdisk_addr);
    //~ fprintf(stdout,"  second_size      :%u\n",      image->header->second_size);
    //~ fprintf(stdout,"  second_addr      :0x%08x\n",  image->header->second_addr);
    //~ fprintf(stdout,"  tags_addr        :0x%08x\n",  image->header->tags_addr);
    //~ fprintf(stdout,"  page_size        :%u\n",      image->header->page_size);       
...
//~ 
    //~ fprintf(stdout,"  id[0]            :%u\n",image->header->id[0]);
    //~ fprintf(stdout,"  id[1]            :%u\n",image->header->id[1]);
    //~ fprintf(stdout,"  id[2]            :%u\n",image->header->id[2]);
    //~ fprintf(stdout,"  id[3]            :%u\n",image->header->id[3]);
    //~ fprintf(stdout,"  id[4]            :%u\n",image->header->id[4]);
    //~ fprintf(stdout,"  id[5]            :%u\n",image->header->id[5]);
    //~ fprintf(stdout,"  id[6]            :%u\n",image->header->id[6]);
    //~ fprintf(stdout,"  id[7]            :%u\n",image->header->id[7]);
    //~ return 0;
//~ }
//~ 
//~ 
...
//~ 
    ...
    //~ // and the image total_size equal to the filesize
    //~ image->data = boot_image_addr;
    //~ image->data_size = boot_image_size;
    //~ 
    //~ //  Search
    //~ D("boot_image_size=%u\n",boot_image_size);
    //~ unsigned char * magic_offset_p = find_in_memory(image->data,image->data_size,BOOT_MAGIC, BOOT_MAGIC_SIZE );
    //~ if(!magic_offset_p){
        //~ image->data = NULL;
        //~ errno = ENOEXEC;
        //~ return ENOEXEC;
 ...
 * 
 * Details : This function will free up all the memory which may of
 * been allocated while handling a boot image
 * 
 * As well as the main boot_image structure allocation
 * image->data - contains the raw bootimage data 
 * 
 * */
void boot_image_free(boot_image* image){
	...
	// 
	if(image->data){
		D("FREEING BOOTIMAGE DATA MEMORY %p\n",image->data );
		free(image->data) ;
	}
	
	if(image){
		D("FREEING BOOTIMAGE MEMORY %p\n",image );
		free(image); 
	}
}


/* abu_bootimage_init - allocates and returns a pointer to a boot_image
 * structure */
...
unsigned int abu_bootimage_open_archive(boot_image* image,const char* filename){
	...
	unsigned archive_file_size = 0;
    unsigned char* archive_file_data = read_item_from_disk(filename,&archive_file_size);
    zipfile_t zip;
    zipentry_t entry;
    zip = init_zipfile(archive_file_data, archive_file_size);
     dump_zipfile(stdout, zip);
     entry = lookup_zipentry(zip, "boot.img");
     ulong_t unsize = get_zipentry_size(entry);
     D("FREEING BOOTIMAGE MEMORY %ld\n",unsize );
     free(archive_file_data );
